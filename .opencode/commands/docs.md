...
| `CHANGELOG.md` | 版本变更日志 |
| `DEPLOY.md` | 部署指南 |
| `.opencode/rules/*.md` | opencode 领域规则 |
| `docs/` 目录 | 其他补充文档 |
...
部署操作指南，与 `.opencode/rules/ci-cd.md` 保持一致。
...
2. **列出目标文件** — 检查所有文档是否存在
   ```
   !`ls -1 README.md AGENTS.md DESIGN.md CHANGELOG.md DEPLOY.md 2>/dev/null`
   ```
